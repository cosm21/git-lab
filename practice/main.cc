...
int main()
{
    ...
    vector<fp> randoms = { get_random_getrandom, get_random_dev_urandom, get_random_rdrand, get_random_std, get_random_env};

    for (int i = 0; i < 5; i++)
    {
        if (get_random(randoms[i]) == 150)
        {
            n += 1;
        }
        else
